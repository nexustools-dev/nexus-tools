...
function csvToJson(
  csvStr: string,
  delimiter: string,
): { json: string; rows: number; cols: number } | { error: string } {
...
export function JsonCsvConverter() {
  ...
  const output =
    result && !('error' in result)
      ? mode === 'json2csv'
        ? (result as { csv: string }).csv
        : (result as { json: string }).json
      : '';
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-zinc-800 overflow-hidden">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors ${mode === 'json2csv' ? 'bg-emerald-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
          >
            {t('jsonToCSV')}
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors ${mode === 'csv2json' ? 'bg-emerald-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
          >
            {t('csvToJSON')}
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${delimiter === v ? 'bg-emerald-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}
            >
              {l}
            </button>
          ))}
            ...
            <div className="flex gap-2">
              {output && (
                <>
                  ...
                  <button
                    onClick={() => download(output, mode === 'json2csv' ? 'csv' : 'json')}
                    className="px-2 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-xs font-medium transition-colors"
                  >
