...
const PRESETS: { key: string; radii: Radii; unit: 'px' | '%' }[] = [
  ...
  {
    key: 'circle',
    radii: { topLeft: 50, topRight: 50, bottomRight: 50, bottomLeft: 50 },
    unit: '%',
  },
  {
    key: 'pill',
    radii: { topLeft: 9999, topRight: 9999, bottomRight: 9999, bottomLeft: 9999 },
    unit: 'px',
  },
...
export function BorderRadiusGenerator() {
  ...
  const applyPreset = useCallback((preset: (typeof PRESETS)[number]) => {
    ...
    setUnit(preset.unit);
    setLinked(
      preset.radii.topLeft === preset.radii.topRight &&
        preset.radii.topRight === preset.radii.bottomRight &&
        preset.radii.bottomRight === preset.radii.bottomLeft,
    );
  }, []);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex rounded-lg border border-zinc-800 overflow-hidden">
              <button
                onClick={() => setUnit('px')}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${unit === 'px' ? 'bg-pink-600 text-white' : 'bg-zinc-900 text-zinc-400'}`}
              >
                {t('pixels')}
              </button>
              <button
                onClick={() => setUnit('%')}
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${unit === '%' ? 'bg-pink-600 text-white' : 'bg-zinc-900 text-zinc-400'}`}
              >
                {t('percent')}
              </button>
            </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
        ...
        <code className="block font-mono text-sm text-pink-300 break-all" translate="no">
          {cssOutput}
        </code>
      </div>
