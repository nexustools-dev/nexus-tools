...
/* ── Presets ── */
const PRESETS: { name: string; type: "linear" | "radial"; angle: number; stops: Omit<ColorStop, "id">[] }[] = [
  { name: "Ocean", type: "linear", angle: 135, stops: [{ color: "#667eea", position: 0 }, { color: "#764ba2", position: 100 }] },
  { name: "Sunset", type: "linear", angle: 90, stops: [{ color: "#fa709a", position: 0 }, { color: "#fee140", position: 100 }] },
  ...
  { name: "Arctic", type: "linear", angle: 135, stops: [{ color: "#74ebd5", position: 0 }, { color: "#ACB6E5", position: 100 }] },
  { name: "Midnight", type: "linear", angle: 135, stops: [{ color: "#0f0c29", position: 0 }, { color: "#302b63", position: 50 }, { color: "#24243e", position: 100 }] },
  { name: "Peach", type: "radial", angle: 0, stops: [{ color: "#ffecd2", position: 0 }, { color: "#fcb69f", position: 100 }] },
  { name: "Aurora", type: "linear", angle: 135, stops: [{ color: "#a8edea", position: 0 }, { color: "#fed6e3", position: 100 }] },
  { name: "Cosmic", type: "linear", angle: 45, stops: [{ color: "#ff6a88", position: 0 }, { color: "#7c4dff", position: 50 }, { color: "#448aff", position: 100 }] },
...
export function CssGradientGenerator() {
  ...
  const addStop = () => {
    ...
    setStops((prev) => [...prev, { id: nextId++, color: randomHex(), position: Math.min(pos, 100) }]);
  };
  ...
  const randomGradient = () => {
    ...
    for (let i = 0; i < count; i++) {
      newStops.push({ id: nextId++, color: randomHex(), position: Math.round((i / (count - 1)) * 100) });
    }
    setStops(newStops);
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-4">
          {/* Type toggle */}
          <div>
            <label className="block text-xs text-zinc-500 mb-2 uppercase tracking-wide">{t("linear")} / {t("radial")}</label>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs text-zinc-500 uppercase tracking-wide">{t("colorStops")}</label>
            {stops.length < 5 && (
              <button
                onClick={addStop}
      ...
      {/* CSS Output */}
      <div>
        <label className="block text-xs text-zinc-500 mb-2 uppercase tracking-wide">{t("cssOutput")}</label>
