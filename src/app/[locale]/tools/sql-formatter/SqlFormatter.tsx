...
import { formatSql, highlightSql, minifySql } from './sql-engine';
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {INDENT_OPTIONS.map((opt) => (
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${indent === opt.value ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}
            >
              {opt.label}
            </button>
          ))}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => copyText(formatted, 'out')}
      ...
      {formatted && (
        <div className="flex gap-4 text-xs text-zinc-500">
          <span>
            {highlighted.lineCount} {t('lineCount')}
          </span>
          <span>
            {highlighted.keywordCount} {t('keywordCount')}
          </span>
        </div>
      )}
    </div>
