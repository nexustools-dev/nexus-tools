...
function md5Bytes(input: Uint8Array): Uint8Array {
  ...
  const result = new Uint8Array(16);
  const rdv = new DataView(result.buffer);
  rdv.setUint32(0, a0, true); rdv.setUint32(4, b0, true);
  rdv.setUint32(8, c0, true); rdv.setUint32(12, d0, true);
  return result;
}
...
function hmacMd5(key: string, message: string): string {
  ...
  const keyBytes = rawKey.length > BLOCK
    ? md5Bytes(new Uint8Array(rawKey))
...
const SECURITY_COLORS: Record<SecurityLevel, { dot: string; badge: string }> = {
  ...
  strongest:{ dot: "bg-emerald-300", badge: "bg-emerald-900/50 text-emerald-300 border-emerald-800" },
};
...
export function HashGenerator() {
  ...
  const computeHashes = useCallback(async (text: string, useHmac: boolean, key: string) => {
    ...
    if (useHmac) {
      ...
      setHashes([
        { algorithm: "HMAC-MD5", noteKey: "hmacMd5Note", hash: hmacMd5(key, text), bits: 128, security: "broken" },
        { algorithm: "HMAC-SHA1", noteKey: "hmacSha1Note", hash: hSha1, bits: 160, security: "weak" },
        { algorithm: "HMAC-SHA256", noteKey: "hmacSha256Note", hash: hSha256, bits: 256, security: "strong" },
        { algorithm: "HMAC-SHA512", noteKey: "hmacSha512Note", hash: hSha512, bits: 512, security: "strongest" },
      ]);
    } else {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-zinc-500 mb-1.5">
                    {t(h.noteKey as "md5Note" | "sha1Note" | "sha256Note" | "sha512Note" | "hmacMd5Note" | "hmacSha1Note" | "hmacSha256Note" | "hmacSha512Note")}
                  </p>
