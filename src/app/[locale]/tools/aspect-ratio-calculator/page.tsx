...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'aspectRatioCalculator.metadata' });
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords').split(', '),
    alternates: {
      canonical: `/${locale}/tools/aspect-ratio-calculator`,
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/aspect-ratio-calculator`])
      ),
    },
  };
}
...
export default async function AspectRatioCalculatorPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'aspectRatioCalculator' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/aspect-ratio-calculator`}
        faq={faq}
      />
