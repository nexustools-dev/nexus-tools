...
interface Resolution { name: string; width: number; height: number; }
...
function getResolutions(rw: number, rh: number): Resolution[] {
  ...
  return targets.map((h) => {
    ...
    const names: Record<number, string> = { 480: "SD", 720: "HD", 1080: "Full HD", 1440: "2K", 2160: "4K", 4320: "8K" };
...
export function AspectRatioCalculator() {
  ...
  const previewScale = useMemo(() => {
    ...
    return { w: Math.max(40, Math.round(width * scale)), h: Math.max(30, Math.round(height * scale)) };
  }, [width, height]);

  const handleWidth = useCallback((val: number) => {
  ...
  const handleHeight = useCallback((val: number) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-center">
            ...
            <div className="text-4xl font-bold font-mono text-amber-400" translate="no">{ratioStr}</div>
          </div>
          ...
          {cssOutput && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-zinc-500 uppercase tracking-wide">{t("cssOutput")}</label>
                <button onClick={() => copyText(cssOutput, "css")} className="px-2 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-xs font-medium transition-colors">
                  {copiedField === "css" ? tc("copied") : t("copy")}
                </button>
              </div>
              <code className="block font-mono text-sm text-amber-300" translate="no">{cssOutput}</code>
            </div>
          )}
        </div>
        ...
        {/* Preview */}
        <div>
          <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-2">{t("preview") || "Preview"}</label>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg flex items-center justify-center h-60">
            ...
            >
              <span className="text-xs text-amber-400 font-mono" translate="no">{width}×{height}</span>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {PRESETS.map((p) => (
                <button key={p.key} onClick={() => applyPreset(p)} className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-medium transition-colors">
                  <span className="font-mono text-amber-400" translate="no">{p.w}:{p.h}</span>
      ...
      {resolutions.length > 0 && (
        <div>
          <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-2">{t("resolutions")}</label>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
            ...
            {resolutions.map((r) => (
              <div key={r.name} className="grid grid-cols-3 text-sm px-4 py-2 border-b border-zinc-800 last:border-b-0 hover:bg-zinc-800/30">
                <span className="text-zinc-400">{r.name}</span>
                <span className="font-mono text-zinc-300" translate="no">{r.width} × {r.height}</span>
                <span className="text-zinc-500 font-mono">{(r.width * r.height / 1000000).toFixed(1)}MP</span>
              </div>
            ))}
          </div>
        </div>
