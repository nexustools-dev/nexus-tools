...
const KNOWN_CLAIMS = ["iss", "sub", "aud", "exp", "nbf", "iat", "jti", "name", "email", "role", "scope"];
...
export function JwtDecoder() {
  ...
  const isExpired = decoded && "exp" in decoded && decoded.exp !== undefined && !("error" in decoded)
  ...
  return (
    <div className="space-y-6">
      ...
      {decoded && !("error" in decoded) && decoded.exp !== null && (
        ...
        >
          <span className={`w-2.5 h-2.5 rounded-full ${isExpired ? "bg-red-400" : "bg-emerald-400"}`} />
          {isExpired
      ...
      {decoded && "error" in decoded && (
        <div className="bg-red-900/50 border border-red-800 rounded-lg px-4 py-3 text-sm text-red-400">
          {decoded.error === "invalidJwt"
            ? t("invalidJwt")
            : t("invalidJson", { part: decoded.error === "invalidJsonHeader" ? "Header" : "Payload" })}
        </div>
      )}
      ...
      {decoded && !("error" in decoded) && (
        <div>
          <label className="block text-xs text-zinc-500 mb-2 uppercase tracking-wide">{t("claims")}</label>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 text-left">
                  ...
                  <th className="px-4 py-2 text-xs text-zinc-500 font-medium hidden md:table-cell">{t("description")}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={key} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                      ...
                      <td className="px-4 py-2 text-zinc-500 text-xs hidden md:table-cell">
                        {descKey ? t(`claimDescriptions.${descKey}` as "claimDescriptions.iss") : ""}
                      </td>
                    </tr>
                  );
                })}
