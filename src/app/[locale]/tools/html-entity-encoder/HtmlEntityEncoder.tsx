...
const ENCODE_MAP: Record<string, string> = {
  ...
  "…": "&hellip;", "•": "&bull;", "°": "&deg;", "±": "&plusmn;", "×": "&times;",
  "÷": "&divide;", "€": "&euro;", "£": "&pound;", "¥": "&yen;", "¢": "&cent;",
  "§": "&sect;", "¶": "&para;", "†": "&dagger;", "‡": "&Dagger;", "←": "&larr;",
  "→": "&rarr;", "↑": "&uarr;", "↓": "&darr;", "♠": "&spades;", "♣": "&clubs;",
  "♥": "&hearts;", "♦": "&diams;", " ": "&nbsp;",
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-zinc-800 overflow-hidden">
          <button onClick={() => { setMode("encode"); setInput(SAMPLE_TEXT); }} className={`px-4 py-2 text-sm font-medium transition-colors ${mode === "encode" ? "bg-rose-600 text-white" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
            {t("encode")}
          </button>
          <button onClick={() => { setMode("decode"); setInput("&lt;p&gt;Hello &amp; welcome&lt;/p&gt;"); }} className={`px-4 py-2 text-sm font-medium transition-colors ${mode === "decode" ? "bg-rose-600 text-white" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
            {t("decode")}
          </button>
        </div>
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              {(["named", "decimal", "hex"] as const).map((m) => (
                <button key={m} onClick={() => setEncodeMode(m)} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${encodeMode === m ? "bg-rose-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>
                  {t(m)}
                </button>
              ))}
            </div>
            <button onClick={() => setEncodeAll(!encodeAll)} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${encodeAll ? "bg-rose-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>
              {t("encodeAll")}
            </button>
          </>
        )}
      ...
      <div className="flex gap-4 text-xs text-zinc-500">
        ...
        <span>{stats.entities} {t("entities")}</span>
      </div>

      {/* Reference table */}
      <div>
        <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-2">{t("reference")}</label>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
          <div className="grid grid-cols-5 text-xs text-zinc-500 font-medium px-4 py-2 border-b border-zinc-800">
            <span>{t("charCol")}</span><span>{t("namedCol")}</span><span>{t("decimalCol")}</span><span>{t("hexCol")}</span><span>{t("descCol")}</span>
          </div>
          {REFERENCE.map((r) => (
            <div key={r.named} className="grid grid-cols-5 text-sm px-4 py-1.5 border-b border-zinc-800 last:border-b-0 hover:bg-zinc-800/30">
              <span className="font-mono text-zinc-200" translate="no">{r.char === " " ? "⎵" : r.char}</span>
              <code className="text-rose-300 text-xs" translate="no">{r.named}</code>
              <code className="text-zinc-400 text-xs" translate="no">{r.decimal}</code>
              <code className="text-zinc-400 text-xs" translate="no">{r.hex}</code>
