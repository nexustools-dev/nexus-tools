...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'glassmorphismGenerator.metadata' });
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords').split(', '),
    alternates: {
      canonical: `/${locale}/tools/glassmorphism-generator`,
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/glassmorphism-generator`])
      ),
    },
  };
}
...
export default async function GlassmorphismGeneratorPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'glassmorphismGenerator' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/glassmorphism-generator`}
        faq={faq}
      />
