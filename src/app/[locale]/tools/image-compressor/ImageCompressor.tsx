  ...
  const compress = useCallback(async (file: File) => {
    ...
    try {
      ...
      const blob = await new Promise<Blob>((resolve, reject) => {
        canvas.toBlob(
          (b) => (b ? resolve(b) : reject(new Error("compressError"))),
          mimeType,
          q
        );
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          {(["jpeg", "webp", "png"] as const).map((f) => (
            <button key={f} onClick={() => setFormat(f)} className={`px-3 py-1.5 rounded text-xs font-medium uppercase transition-colors ${format === f ? "bg-emerald-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>
              {f}
            </button>
          ))}
        </div>
        {format !== "png" && (
          <div className="flex items-center gap-2">
            ...
            <input type="range" min={10} max={100} value={quality} onChange={(e) => setQuality(Number(e.target.value))} className="w-32 accent-emerald-500" />
            <span className="text-xs text-zinc-400 font-mono w-8">{quality}%</span>
          </div>
        )}
        <div className="flex items-center gap-2">
          <label className="text-xs text-zinc-500">{t("maxWidth")}:</label>
          <select value={maxWidth} onChange={(e) => setMaxWidth(Number(e.target.value))} className="bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300">
      ...
      >
        ...
        <div className="text-zinc-400 text-sm">
          {processing ? t("processing") : t("dropzone")}
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-center">
              ...
              <div className="text-lg font-mono text-zinc-300">{formatSize(result.originalSize)}</div>
            </div>
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-center">
              <div className="text-xs text-zinc-500 mb-1">{t("compressed")}</div>
              <div className="text-lg font-mono text-emerald-400">{formatSize(result.compressedSize)}</div>
            </div>
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-center">
              <div className="text-xs text-zinc-500 mb-1">{t("savings")}</div>
              <div className={`text-lg font-mono ${savings > 0 ? "text-emerald-400" : "text-amber-400"}`}>{savings}%</div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
                <img src={result.originalUrl} alt="Original" className="w-full h-auto max-h-64 object-contain" />
              </div>
            </div>
            <div>
              <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-2">{t("compressed")}</label>
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
                <img src={result.compressedUrl} alt="Compressed" className="w-full h-auto max-h-64 object-contain" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={download} className="px-6 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-sm font-medium transition-colors">
