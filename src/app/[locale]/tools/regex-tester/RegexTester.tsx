  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {error && <p className="text-xs text-red-400 mt-1">{t('invalidRegex', { error })}</p>}
      </div>
            ...
            {matches.map((m, idx) => (
              <div key={idx} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
                <div className="flex items-center gap-3 text-xs">
                  <span
                    className={`px-2 py-0.5 rounded font-mono ${MATCH_COLORS[idx % MATCH_COLORS.length]}`}
                  >
                    {t('fullMatch')}
                  </span>
      ...
      <div>
        ...
        >
          <span className={`transition-transform ${showCheatSheet ? 'rotate-90' : ''}`}>▶</span>
          {t('cheatSheet')}
        </button>
        {showCheatSheet && (
