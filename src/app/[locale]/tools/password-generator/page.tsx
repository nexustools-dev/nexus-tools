...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'passwordGenerator.metadata' });
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords').split(', '),
    alternates: {
      canonical: `/${locale}/tools/password-generator`,
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/password-generator`])
      ),
    },
  };
}
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/password-generator`}
        faq={faq}
      />
