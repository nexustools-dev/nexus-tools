...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'cssUnitConverter.metadata' });
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords').split(', '),
    alternates: {
      canonical: `/${locale}/tools/css-unit-converter`,
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/css-unit-converter`])
      ),
    },
  };
}
...
export default async function CssUnitConverterPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'cssUnitConverter' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/css-unit-converter`}
        faq={faq}
      />
