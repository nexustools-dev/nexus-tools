...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'jsonYamlConverter.metadata' });
  return {
    title: t('title'),
...
export default async function JsonYamlConverterPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'jsonYamlConverter' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/json-yaml-converter`}
        faq={faq}
      />
