...
function jsonToYaml(value: unknown, indentStr: string, depth: number, inlineArrays: boolean): string {
  ...
  if (Array.isArray(value)) {
    ...
    if (inlineArrays && value.length <= 8 && value.every((v) => v === null || typeof v !== "object")) {
...
function convertJsonToYaml(jsonStr: string, indentStr: string, inlineArrays: boolean): { yaml: string; keys: number; depth: number } | { error: string } {
  try {
...
interface YamlLine { indent: number; key: string; value: string; isArrayItem: boolean; raw: string; }
...
function parseYamlLines(yaml: string): YamlLine[] {
  return yaml.split("\n").filter((l) => l.trim() && !l.trim().startsWith("#")).map((line) => {
    ...
    if (colonIdx > 0 && !trimmed.startsWith("- ")) {
      return { indent, key: trimmed.slice(0, colonIdx).replace(/^["']|["']$/g, ""), value: trimmed.slice(colonIdx + 2), isArrayItem, raw };
    }
    if (trimmed.endsWith(":")) {
      return { indent, key: trimmed.slice(0, -1).replace(/^["']|["']$/g, ""), value: "", isArrayItem, raw };
    }
  ...
  function parseBlock(start: number, baseIndent: number): { value: unknown; next: number } {
    ...
    while (i < lines.length && lines[i].indent === baseIndent && !lines[i].isArrayItem) {
      ...
      if (item.key && item.value) {
        obj[item.key] = parseYamlValue(item.value);
        i++;
      } else if (item.key && !item.value && i + 1 < lines.length && lines[i + 1].indent > baseIndent) {
...
function convertYamlToJson(yaml: string, indentSize: number): { json: string; keys: number; depth: number } | { error: string } {
  try {
...
function countKeys(obj: unknown): number {
  ...
  return Object.entries(obj as Record<string, unknown>).reduce(
    (acc, [, v]) => acc + 1 + countKeys(v), 0
  );
}

function calcDepth(obj: unknown, d = 0): number {
  if (obj === null || typeof obj !== "object") return d;
  if (Array.isArray(obj)) return obj.length === 0 ? d + 1 : Math.max(...obj.map((v) => calcDepth(v, d + 1)));
...
export function JsonYamlConverter() {
  ...
  const output = result && !("error" in result)
    ? ("yaml" in result ? result.yaml : result.json)
    : "";
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-zinc-800 overflow-hidden">
          <button onClick={() => { setMode("json2yaml"); setInput(SAMPLE_JSON); }} className={`px-4 py-2 text-sm font-medium transition-colors ${mode === "json2yaml" ? "bg-orange-600 text-white" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
            {t("jsonToYaml")}
          </button>
          <button onClick={() => { setMode("yaml2json"); setInput(SAMPLE_YAML); }} className={`px-4 py-2 text-sm font-medium transition-colors ${mode === "yaml2json" ? "bg-orange-600 text-white" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
            {t("yamlToJson")}
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {[2, 4].map((n) => (
            <button key={n} onClick={() => setIndentSize(n)} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${indentSize === n ? "bg-orange-600 text-white" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>
              {n}
            </button>
          ))}
        </div>
        ...
        <button onClick={() => setInput("")} className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm font-medium transition-colors ml-auto">
          {t("clear")}
        </button>
      </div>
            ...
            <div className="flex gap-2">
              {output && (
                <>
                  ...
                  <button onClick={() => download(output, mode === "json2yaml" ? "yaml" : "json")} className="px-2 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-xs font-medium transition-colors">
      ...
      {result && !("error" in result) && (
        <div className="flex gap-4 text-xs text-zinc-500">
          <span>{result.keys} {t("keysCount")}</span>
          <span>{t("maxDepth")}: {result.depth}</span>
        </div>
      )}
    </div>
  );
