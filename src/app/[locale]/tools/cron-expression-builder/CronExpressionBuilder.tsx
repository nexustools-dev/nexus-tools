...
function describeCron(cron: string): string {
  ...
  if (hour.includes('/')) pieces.push(`every ${hour.split('/')[1]} hours`);
    ...
    pieces.push(`at ${hour.padStart(2, '0')}:${min === '*' ? '00' : min.padStart(2, '0')}`);
...
export function CronExpressionBuilder() {
  ...
  const nextExecs = useMemo(
    () => (fieldsValid ? getNextExecutions(cron, 5) : []),
    [cron, fieldsValid],
  );
        ...
        <div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden divide-y divide-zinc-800">
            {nextExecs.map((d, i) => (
              <div
                key={i}
                className="flex items-center justify-between px-4 py-2.5 hover:bg-zinc-800/30"
              >
