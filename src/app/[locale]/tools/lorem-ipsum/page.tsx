...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'loremIpsum.metadata' });
  return {
    title: t('title'),
    ...
    alternates: {
      ...
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/lorem-ipsum`])
      ),
    },
  };
}
...
export default async function LoremIpsumPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'loremIpsum' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/lorem-ipsum`}
        faq={faq}
      />
