...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'colorPaletteGenerator.metadata' });
  return {
    title: t('title'),
    ...
    alternates: {
      ...
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/color-palette-generator`])
      ),
    },
  };
}

export default async function ColorPaletteGeneratorPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'colorPaletteGenerator' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/color-palette-generator`}
        faq={faq}
      />
