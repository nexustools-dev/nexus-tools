...
interface Shadow { x: number; y: number; blur: number; color: string; opacity: number; }
...
const PRESETS: { key: string; shadows: Shadow[]; textColor: string; bgColor: string }[] = [
  { key: "simple", shadows: [{ x: 2, y: 2, blur: 4, color: "#000000", opacity: 50 }], textColor: "#ffffff", bgColor: "#18181b" },
  { key: "glow", shadows: [{ x: 0, y: 0, blur: 20, color: "#3b82f6", opacity: 80 }, { x: 0, y: 0, blur: 40, color: "#3b82f6", opacity: 40 }], textColor: "#60a5fa", bgColor: "#18181b" },
  { key: "neon", shadows: [{ x: 0, y: 0, blur: 10, color: "#22d3ee", opacity: 100 }, { x: 0, y: 0, blur: 20, color: "#22d3ee", opacity: 60 }, { x: 0, y: 0, blur: 40, color: "#22d3ee", opacity: 30 }], textColor: "#ecfeff", bgColor: "#0f172a" },
  { key: "retro", shadows: [{ x: 3, y: 3, blur: 0, color: "#ef4444", opacity: 100 }, { x: 6, y: 6, blur: 0, color: "#f97316", opacity: 100 }], textColor: "#fef3c7", bgColor: "#1e1b4b" },
  { key: "emboss", shadows: [{ x: -1, y: -1, blur: 1, color: "#ffffff", opacity: 30 }, { x: 1, y: 1, blur: 1, color: "#000000", opacity: 50 }], textColor: "#71717a", bgColor: "#3f3f46" },
  { key: "fire", shadows: [{ x: 0, y: 0, blur: 10, color: "#ef4444", opacity: 80 }, { x: 0, y: -5, blur: 20, color: "#f97316", opacity: 60 }, { x: 0, y: -10, blur: 40, color: "#eab308", opacity: 30 }], textColor: "#fef08a", bgColor: "#18181b" },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <input type="range" min={16} max={96} value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className="w-20 accent-cyan-500" />
          ...
          <div className="flex gap-3 mt-2">
            <div className="flex items-center gap-2">
              ...
              <input type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)} className="w-8 h-8 rounded cursor-pointer bg-transparent border-0" />
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={addShadow} disabled={shadows.length >= 5} className="px-3 py-1.5 rounded bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 disabled:cursor-not-allowed text-xs font-medium transition-colors">
              + {t("addShadow")}
            </button>
          </div>
          ...
          <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
            {shadows.map((s, idx) => (
              <div key={idx} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 space-y-2">
                ...
                {([["x", "offsetX", -50, 50], ["y", "offsetY", -50, 50], ["blur", "blur", 0, 100]] as const).map(([key, label, min, max]) => (
                  <div key={key} className="flex items-center gap-3">
                    <label className="text-xs text-zinc-500 w-10 shrink-0">{t(label)}</label>
                    <input type="range" min={min} max={max} value={s[key]} onChange={(e) => updateShadow(idx, key, Number(e.target.value))} className="flex-1 accent-cyan-500" />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input type="range" min={0} max={100} value={s.opacity} onChange={(e) => updateShadow(idx, "opacity", Number(e.target.value))} className="flex-1 accent-cyan-500" />
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
        ...
        <code className="block font-mono text-sm text-cyan-300 break-all" translate="no">{cssOutput}</code>
      </div>
