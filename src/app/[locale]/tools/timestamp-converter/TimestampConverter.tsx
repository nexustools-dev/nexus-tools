...
function formatInTz(ms: number, tz: string, style: "full" | "iso" | "rfc" | "utc" | "local"): string {
  const d = new Date(ms);
  switch (style) {
    case "iso":
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs text-zinc-500 uppercase tracking-wide">{t("currentTimestamp")}</label>
      ...
      {/* Timezone selector */}
      <div>
        <label className="block text-xs text-zinc-500 mb-2 uppercase tracking-wide">{t("timezone")}</label>
        <select
          value={tz}
      ...
      {parsed !== null && (
        <div>
          <label className="block text-xs text-zinc-500 mb-2 uppercase tracking-wide">{t("formats")}</label>
