  ...
  const parse = (line: string): string[] => {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      if (inQuote) {
        ...
        if (ch === '"') { inQuote = true; }
        else if (ch === delimiter) { result.push(cur.trim()); cur = ""; }
        else { cur += ch; }
      }
    }
...
function generateInserts(
  ...
  if (createTable) {
    ...
    if (dialect === "mssql") {
      ...
      parts.push(`CREATE TABLE ${dialect === "mysql" ? "IF NOT EXISTS " : ""}${quoteIdentifier(tableName, dialect)} (\n${cols}\n);\n`);
    }
  }
  ...
  if (batchSize <= 1 || dialect === "mssql") {
    for (const row of rows) {
      ...
      parts.push(`INSERT INTO ${quoteIdentifier(tableName, dialect)} (${qHeaders.join(", ")}) VALUES (${vals});`);
    }
  } else {
    for (let i = 0; i < rows.length; i += batchSize) {
      const batch = rows.slice(i, i + batchSize);
      const valLines = batch.map((row) => `  (${row.map((v) => escapeValue(v, dialect)).join(", ")})`).join(",\n");
      parts.push(`INSERT INTO ${quoteIdentifier(tableName, dialect)} (${qHeaders.join(", ")}) VALUES\n${valLines};`);
    }
  }
...
export function CsvToSql() {
  ...
  const sql = useMemo(
    () => headers.length > 0 && rows.length > 0 ? generateInserts(headers, rows, tableName || "my_table", dialect, batchSize, createTable) : "",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={delimiter} onChange={(e) => setDelimiter(e.target.value)} className="bg-zinc-900 border border-zinc-800 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-emerald-500">
            <option value=",">,</option>
            <option value=";">;</option>
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={batchSize} onChange={(e) => setBatchSize(Number(e.target.value))} className="bg-zinc-900 border border-zinc-800 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-emerald-500">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={createTable} onChange={(e) => setCreateTable(e.target.checked)} className="accent-emerald-500" />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-zinc-600">{rows.length} {t("rows")} × {headers.length} {t("columns")}</span>
          </div>
          <textarea
            value={csv}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs text-zinc-500 uppercase tracking-wide">{t("sqlOutput")}</label>
            <button
          ...
          <pre className="w-full h-64 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 font-mono text-sm overflow-auto text-emerald-300 whitespace-pre-wrap" translate="no">
      ...
      {headers.length > 0 && rows.length > 0 && (
        <div>
          <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-2">{t("preview")} ({Math.min(rows.length, 5)}/{rows.length})</label>
          <div className="overflow-x-auto border border-zinc-800 rounded-lg">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-zinc-800/50">
                  {headers.map((h, i) => (
                    <th key={i} className="px-3 py-2 text-left text-xs text-zinc-400 font-medium">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.slice(0, 5).map((row, ri) => (
                  <tr key={ri} className="border-t border-zinc-800/50 hover:bg-zinc-800/20">
                    {row.map((cell, ci) => (
                      <td key={ci} className="px-3 py-1.5 text-zinc-300 font-mono text-xs">{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
