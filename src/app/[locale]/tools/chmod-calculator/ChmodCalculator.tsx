...
interface Permissions { owner: number; group: number; others: number; special: number; }
...
function toSymbolic(p: Permissions): string {
  let s = permToSymbolic(p.owner) + permToSymbolic(p.group) + permToSymbolic(p.others);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-sm text-zinc-400">
          ...
          <code className="font-mono text-lg text-green-400" translate="no">{symbolic}</code>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
        ...
        <div className="grid grid-cols-4 gap-0 border-b border-zinc-800">
          ...
          {PERM_BITS.map((p) => (
            <div key={p.label} className="px-4 py-3 text-center text-xs text-zinc-500 uppercase font-medium">
              {t(p.label)}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-0">
          <div className="px-4 py-3 text-sm font-medium text-zinc-400">{t("special") || "Special"}</div>
          {SPECIAL_BITS.map((s) => {
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <code className="block font-mono text-lg text-zinc-200 mt-1" translate="no">{command}</code>
          </div>
          <button onClick={() => copyText(command, "cmd")} className="px-3 py-1.5 rounded bg-zinc-800 hover:bg-zinc-700 text-sm font-medium transition-colors">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {PRESETS.map((p) => (
            ...
            >
              <span className="font-mono text-green-400" translate="no">{toOctal(p.perms)}</span>
