...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'chmodCalculator.metadata' });
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords').split(', '),
    alternates: {
      canonical: `/${locale}/tools/chmod-calculator`,
      languages: Object.fromEntries(
        (['en', 'es', 'pt'] as const).map((l) => [l, `/${l}/tools/chmod-calculator`])
      ),
    },
  };
}
...
export default async function ChmodCalculatorPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'chmodCalculator' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <ToolJsonLd
        ...
        url={`https://toolnexus.dev/${locale}/tools/chmod-calculator`}
        faq={faq}
      />
